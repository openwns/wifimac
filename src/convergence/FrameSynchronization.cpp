...
FrameSynchronization::FrameSynchronization(wns::ldk::fun::FUN* fun, const wns::pyconfig::View& config):
    ...
    slcCapture(config.get<wns::Ratio>("slcCapture")),
    slgCapture(config.get<wns::Ratio>("slgCapture")),
    idleCapture(config.get<wns::Ratio>("idleCapture")),
    detectionThreshold(config.get<wns::Ratio>("detectionThreshold")),
    lastFrameEnd(0),
